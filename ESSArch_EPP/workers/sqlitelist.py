...
'''
    ...
    Copyright (C) 2010-2014  ES Solutions AB, Henrik Ek
...
if _major_version<3: # py <= 2.x
    if version_info[1]<5: # py <= 2.4
        raise ImportError("sqlitelist requires python 2.5 or higher (python 3.3 or higher supported)")
...
class SqliteList(object):
    def __init__(self, filename=None, tablename='unnamed', flag='c',
        ...
        """
        Initialize sqlite-backed list. The list will be a table `tablename` in 
        database file `filename`. A single file (=database) may contain multiple tables.
    ...
    def values(self):
        ...
        return  [decode(value[0]) for value in self.cursor.execute(GET_VALUES)]

    def itervalues(self):
        GET_VALUES = 'SELECT value FROM %s ORDER BY rowid' % self.tablename
        #for value in self.conn.select(GET_VALUES):
        for value in self.cursor.execute(GET_VALUES):
    ...
    def append(self, x=None):
        ...
        self.cursor.execute(UPDATE_ITEMS, (encode(x),))
        if self.autocommit:
            self.conn.commit()
    ...
    def extend(self, l=(),):
        ...
        self.cursor.executemany(UPDATE_ITEMS, items)
        if self.autocommit:
            self.conn.commit()
...
if _major_version == 2:
    #setattr(SqliteList,"iterkeys",lambda self: self.keys())
    #setattr(SqliteList,"itervalues",lambda self: self.values())
    #setattr(SqliteList,"iteritems",lambda self: self.items())
